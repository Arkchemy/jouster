...
>
> This describes an early milestone from before this repository used
> devkitA64/libnx at all, when devkitPro was unreachable from the dev
> environment and the goal was simply "does hbloader accept an `.nro` we
> produced". That constraint no longer applies: `game/Makefile` now requires
> `DEVKITPRO` and links against `libnx` and `deko3d`, and the payload is the
> real recompiled game rather than a trivial stub.
>
> Kept because the Homebrew ABI / startup notes below are still accurate and
> still useful. For current repository documentation see [../README.md](../README.md).
